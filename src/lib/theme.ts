...
export function pickTextColor(bgHex: string): "#000000" | "#ffffff" {
  const cBlack = contrast(bgHex, "#000000"); // prefer >= 4.5 normally
...
function approxGradientMid({ from, via, to }: Palette): string {
  // simple average in RGB space (good enough for contrast decision)
...
export function applyTheme(p?: Palette) {
  ...
  // For your request: always bold when used on gradient
